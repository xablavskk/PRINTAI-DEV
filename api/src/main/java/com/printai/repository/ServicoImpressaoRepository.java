...
public interface ServicoImpressaoRepository extends JpaRepository<ServicoImpressao, Long> {
    ...
    @Query("SELECT s FROM ServicoImpressao s WHERE " +
           "(:tecnologia IS NULL OR LOWER(s.tecnologia) = LOWER(:tecnologia)) AND " +
    ...
    List<ServicoImpressao> buscarAvancado(@Param("tecnologia") String tecnologia, @Param("material") String material, @Param("modelo") String modelo);
    ...
    List<ServicoImpressao> buscarSimplificado(@Param("pecasPequenas") boolean pecasPequenas, @Param("decorativos") boolean decorativos, @Param("prototipos") boolean prototipos);
}
